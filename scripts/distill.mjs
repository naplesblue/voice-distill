...
const inputPath = args.find(a => !a.startsWith('-') && a !== sampleN);
...
try {
  ...
  if (!resume) {
    if (useBear) {
      ...
      log(`▸ Phase 1: 从 ${inputPath} 导入...\n`);
      run('import_folder.mjs', [inputPath]);
    }
    log('');

    // Phase 2: 计算分析
    log('▸ Phase 2: 词汇指纹 + 句法签名（零 LLM）...\n');
    run('distill_compute.mjs');
    log('');
  }
